...
EOF
	$DOTSPLAT_FN track rc-files $HOME/.zshrc > /dev/null
	assertTrue "\`track' did not move the .zshrc file" "[ -f $DOTSPLAT/repos/rc-files/home/.zshrc ]"
...
dotsplat --batch refresh
EOF
	$DOTSPLAT_FN track rc-files $HOME/.path\ with\ spaces > /dev/null
	assertEquals "\`track' did not exit with status 0" 0 $?
	assertTrue "\`track' did not move the \`.path with spaces' file" "[ -f $DOTSPLAT/repos/rc-files/home/.path\ with\ spaces ]"
...
dotsplat --batch refresh
EOF
	$DOTSPLAT_FN track rc-files $HOME/deep\ folder/structure/with\ spaces/.file\ with\ spaces > /dev/null
	assertEquals "\`track' did not exit with status 0" 0 $?
	assertTrue "\`track' did not move the \`.file with spaces' file" "[ -f $DOTSPLAT/repos/rc-files/home/deep\ folder/structure/with\ spaces/.file\ with\ spaces ]"
...
dotsplat --batch refresh
EOF
	$DOTSPLAT_FN track rc-files $HOME/.path\ with\ spaces $HOME/deep\ folder/structure/with\ spaces/.file\ with\ spaces > /dev/null
	assertEquals "\`track' did not exit with status 0" 0 $?
	assertTrue "\`track' did not move the \`.file with spaces' file" "[ -f $DOTSPLAT/repos/rc-files/home/deep\ folder/structure/with\ spaces/.file\ with\ spaces ]"
...
dotsplat --batch refresh
EOF
	(cd $HOME; $DOTSPLAT_FN track rc-files .zshrc) > /dev/null
	assertTrue "\`track' did not move the .zshrc file" "[ -f $DOTSPLAT/repos/rc-files/home/.zshrc ]"
	assertTrue "\`track' did not symlink the .zshrc file" "[ -L $HOME/.zshrc ]"
}
...
My empty vim config
EOF
	(cd $HOME; $DOTSPLAT_FN track repo\ with\ spaces\ in\ name .vimrc) > /dev/null
	local file="$DOTSPLAT/repos/repo with spaces in name/home/.vimrc"
...
dotsplat should refuse to track this file
EOF
	$DOTSPLAT_FN track rc-files $NOTHOME/some_other_file &> /dev/null
...
EOF
	...
	rm $HOME/.zshrc
	cat > $HOME/.zshrc <<EOF
dotsplat --batch refresh 7
EOF
	$DOTSPLAT_FN track rc-files $HOME/.zshrc &> /dev/null
	local tracked_file_size=$(stat -c %s $DOTSPLAT/repos/rc-files/home/.zshrc 2>/dev/null || \
	                          stat -f %z $DOTSPLAT/repos/rc-files/home/.zshrc)
...
EOF
	...
	assertTrue "\`track' did not symlink the .zshrc file" "[ -L $HOME/.zshrc ]"
	$DOTSPLAT_FN track rc-files $HOME/.zshrc &> /dev/null
	assertTrue "\`track' has double tracked the .zshrc file" "[ ! -L $DOTSPLAT/repos/rc-files/home/.zshrc ]"
}
...
EOF
	$DOTSPLAT_FN track rc-files $HOME/.zshrc > /dev/null
	local git_status=$(cd $DOTSPLAT/repos/rc-files; git status --porcelain)
